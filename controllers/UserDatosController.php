...
public function actionGetClinicasByCorporativo($corporativo_id)
{
    \Yii::$app->response->format = Response::FORMAT_JSON;

    // Buscar los "clinica_id" asociados al corporativo en la tabla intermedia
    // AHORA USAMOS EL MODELO Y CAMPO CORRECTOS
    $corporativoClinicas = CorporativoClinica::find()
        ->where(['corporativo_id' => $corporativo_id])
        ->all();

    // Obtener un array de los IDs de las clínicas
    // AHORA BUSCAMOS LA COLUMNA 'clinica_id'
    $clinicaIds = ArrayHelper::getColumn($corporativoClinicas, 'clinica_id');

    // Usar la función del UserHelper para obtener solo esas clínicas
    $clinicas = UserHelper::getClinicasByIds($clinicaIds);

    // Preparar el HTML para el Select2
    $options = '<option value="">Seleccione</option>';
    foreach ($clinicas as $id => $name) {
...
public function actionGetAllClinicas()
{
    // Esta función no necesita cambios, ya que usa el UserHelper que funciona
    \Yii::$app->response->format = Response::FORMAT_JSON;
    $clinicas = UserHelper::getClinicasList();
    $options = '<option value="">Seleccione</option>';
    foreach ($clinicas as $id => $name) {
        $options .= '<option value="' . $id . '">' . $name . '</option>';
    }
